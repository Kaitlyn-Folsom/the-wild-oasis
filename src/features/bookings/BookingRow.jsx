...
function BookingRow({
  ...
  return (
    <Table.Row>
      ...
      <Stacked>
        <span>
          {isToday(new Date(start_date)) ? 'Today' : formatDistanceFromNow(start_date)} &rarr; {num_night} night stay
        </span>
        <span>
      ...
      {status ? <Tag type={statusToTagName[status]}>{status.replace('-', ' ')}</Tag> : <span>&mdash;</span>}
